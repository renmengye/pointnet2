...
parser.add_argument(
    '--max_epoch', type=int, default=251, help='Epoch to run [default: 251]')
parser.add_argument(
    '--batch_size',
    type=int,
    default=16,
...
parser.add_argument(
    '--normal', action='store_true', help='Whether to use normal information')
parser.add_argument(
    '--dropout', action='store_true', help='Whether to dropout input points')
parser.add_argument(
    '--aug', action='store_true', help='Whether to add data augmentation')
...
def train():
    with tf.Graph().as_default():
        ...
        # offset_pl = tf.placeholder(
        #     tf.float32, shape=(BATCH_SIZE, NUM_POINTS, NUM_NEIGHBORS, 3))
        ...
        print([vv.name for vv in tf.trainable_variables()])
        print([
            gg.name if gg is not None else 'none'
            for gg in tf.gradients(total_loss, tf.trainable_variables())
        ])
        ...
        best_acc = -1
        logger = get_exp_logger(os.path.join(LOG_DIR, 'tensorboard'))

        for epoch in range(MAX_EPOCH):
            log_string('**** EPOCH %03d ****' % (epoch))
            sys.stdout.flush()

            train_one_epoch(sess, ops, logger)
            eval_one_epoch(sess, ops, logger)
...
def eval_one_epoch(sess, ops, logger):
    """ ops: dict mapping from string to tf ops """
    global EPOCH_CNT
    is_training = False
    ...
    logger.log('test loss', EPOCH_CNT, loss_sum / float(batch_idx))
    logger.log('test acc', EPOCH_CNT, total_correct / float(total_seen))
    logger.flush()
    EPOCH_CNT += 1
